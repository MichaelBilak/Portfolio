...
1. A public contact form creates a lead.
2. Sales qualifies the lead and either closes it or converts it into a case.
3. A manager assigns the case, stage, deadline, and workflow template.
4. The team works from tasks, the case journal, private files, and versioned
   specifications.
5. Automations create reminders and template tasks. They never make destructive
   changes without an explicit user action.
6. Completed cases remain searchable and auditable.
...
1. Apply CRM migrations `003_crm_backend.sql` and
   `004_fix_case_event_cascades.sql` after the existing Studio schema.
2. Create the private `case-files` storage bucket if the migration cannot do it
   in the current Supabase environment.
...
temporary case with related records and a private file, verifies them, and
removes the test data in a `finally` cleanup.
...
the `x-crm-cron-secret` header. The endpoint deduplicates unread reminders and
runs enabled `task_due` automation rules.
